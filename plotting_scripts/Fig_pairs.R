...
if (FALSE) {
# Sylvie's data
isolates_growth_syl1 <-  isolates_growth_syl %>%
    select(ID = SangerID, cs, gr_max) %>%
    pivot_wider(names_from = cs, values_from = gr_max) %>%
    right_join(select(isolates, Community, Isolate, ID))
pairs_meta <- pairs %>%
    left_join(rename_with(isolates_growth_syl1, ~ paste0(., "1"), !contains("Community"))) %>%
    left_join(rename_with(isolates_growth_syl1, ~ paste0(., "2"), !contains("Community")))
...
pairs_meta_matched <- pairs_meta %>%
    ...
    select(Community, Isolate1, Isolate2, InteractionType, From, To,
           PairFermenter, ID1, ID2,
           PreferredCS1, PreferredCS2, SecretedCS1, SecretedCS2) %>%
...
pivot_longer(cols = -ID) %>%
    ...
    group_by(ID) %>%
    mutate(Order = 1:3) %>%
    pivot_wider(id_cols = ID, names_from = Order, values_from = value) %>%
    filter(`1` == "16hr", `2` == "28hr", `3` == "48hr")
summarize()

filter(value == "16hr") %>%
    summarize(acetate_peak = match(acetate_mM == max(acetate_mM), .$Time))


pairs_meta_matched <- pairs_meta %>%
    # The CS of most abundant secretion for each isolate
    pivot_longer(cols = contains("mM", ignore.case = F), names_to = "CSS", values_to = "Secretion") %>%
    separate(col = CSS, into = c("CSS", "whos_CSS"), sep = -1) %>%
    ## Remove secretion of acetate
    filter(!CSS == "acetate_mM") %>%
    group_by(Community, Isolate1, Isolate2, whos_CSS) %>%
    filter(Secretion == max(Secretion)) %>%
    # The CS high growth rate for each isolate
    pivot_longer(cols = contains(c("Acetate", "D-Glucose", "D-Lactate", "Succinate"), ignore.case = F), names_to = "CSR", values_to = "R") %>%
    separate(col = CSR, into = c("CSR", "whos_CSR"), sep = -1) %>%
    ## Remove R on glucose
    filter(CSR != "D-Glucose") %>%
    group_by(Community, Isolate1, Isolate2, whos_CSR) %>%
    filter(R == max(R)) %>%
    filter(whos_CSS == 1, whos_CSR == 2) %>%
    # Matches
    mutate(CSS = sub("_mM", "", CSS), CSR = sub("2-", "", sub("D-", "", CSR)) %>% tolower()) %>%
    mutate(MatchedCS = CSS == CSR)
...
## OD620_16h_d, on glucose. From Sylvie's deep well data
pairs_meta %>%
    filter(!is.na(PairFermenter)) %>%
...
## Matched CS of secretion by dominant and preferred by subdominant
p5 <- pairs_meta_matched %>%
    filter(!is.na(PairFermenter)) %>%
    ungroup() %>%
...
p6_2 <- pairs_meta %>%
    left_join(isolates_growth_dis) %>%
    filter(!is.na(PairFermenter)) %>%
    ggplot(aes(x = abs(r_glu_d), y = abs(r_dissim), shape = InteractionType, color = PairFermenter)) +
    geom_point(size = 2) +
    scale_shape_manual(values = c("coexistence" = 16, "exclusion" = 1)) +
    scale_color_discrete(labels = c(FF = "Fermenter-Fermenter", FN = "Fermenter-Respirator", NN = "Respirator-Respirator")) +
    theme_classic() +
    theme(legend.title = element_blank(), legend.position = "top", legend.direction = "horizontal") +
    guides(shape = "none") +
    labs(x = expression(r[A_glu] - r[B_glu]), y = expression("|r|"))

p <- plot_grid(p6, p6_2, ncol = 2)

ggsave(here::here("plots/Fig_pairs-r_dissim_scatter.png"), p, width = 10, height = 5)

## Scatterplot of r_glu_d against r_total_d
pairs_meta %>%
    left_join(isolates_growth_dis) %>%
    filter(!is.na(PairFermenter)) %>%
    ggplot() +
...
if (FALSE) {
    ...
    isolates %>%
        ...
        select(ID, Community, Isolate, Family, Genus, Win, Lose, Draw) %>%
        arrange(desc(Win), desc(Draw))

    pairs %>%
        filter(Community == "C11R2") %>%
        select(Community, Isolate1, Isolate2, Family, Genus, Win, Lose, Draw) %>%
        arrange(desc(Win), desc(Draw))
    ...
    pairs_meta %>%
        ...
        #filter(PairFermenter == "FF") %>%
        #mutate(r_glu_d = `D-Glucose1` - `D-Glucose2`, r_ace_d = Acetate1 - Acetate2) %>%
        mutate(r_OD_d = OD6201 - OD6202) %>%
        ggplot() +
        geom_boxplot(aes(x = InteractionType, y= r_OD_d)) +
        geom_jitter(aes(x = InteractionType, y= r_OD_d)) +
        geom_hline(yintercept = 0, color = "red") +
    ...
    pairs_meta %>%
        filter(!is.na(PairFermenter)) %>%
        #filter(PairFermenter == "FF") %>%
        mutate(r_d = Acetate1 - Acetate2) %>%
        ggplot() +
        geom_boxplot(aes(x = InteractionType, y= r_d)) +
        geom_jitter(aes(x = InteractionType, y= r_d)) +
        geom_hline(yintercept = 0, color = "red") +
        facet_grid(.~PairFermenter) +
        theme_bw()
    ...
    plot_boxplot <- function (pairs_meta, metabolite = "Acetate") {
        pairs_meta %>%
            filter(!is.na(PairFermenter)) %>%
            mutate(r_d = {{metabolite}}) %>%
            ggplot() +
            geom_boxplot(aes(x = InteractionType, y= r_d)) +
            geom_jitter(aes(x = InteractionType, y= r_d)) +
            geom_hline(yintercept = 0, color = "red") +
    ...
    plot_boxplot(pairs_meta, metabolite = "Acetate")
    ...
    pairs_meta %>%
        ...
        mutate(r_glu_d = `D-Glucose1` - `D-Glucose2`, r_ace_d = Acetate1 - Acetate2) %>%
        ggplot() +
        geom_boxplot(aes(x = InteractionType, y= r_glu_d)) +
        geom_jitter(aes(x = InteractionType, y= r_glu_d)) +
        geom_hline(yintercept = 0, color = "red") +
        theme_bw()
    ...
    pairs_meta %>%
        pivot_longer(cols = ends_with("2") & !contains("Isolate") & !contains("Fermenter") & !contains("Family") & !contains("Genus"), names_to = "metabolite2", values_to = "value2") %>%
        filter(!is.na(PairFermenter)) %>%
        ggplot() +
        geom_point(aes(x = `D-Glucose1`, y = value2, color = PairFermenter, shape = InteractionType), size = 2) +
        geom_abline(slope = 1, intercept = 0) +
        scale_shape_manual(values = c(coexistence = 16, exclusion = 1, neutrality = 17)) +
        facet_wrap(.~metabolite2, scales = "free_y") +
        theme_bw() +
        labs(x = expression("r"[glucose, A]), y = expression("r"[CS, B]))
