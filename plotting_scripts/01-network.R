...
ggsave(here::here("plots/Fig1A-example_network.png"), pA, width = 5, height = 5)

# Figure 1B: All networks
...
ggsave(here::here("plots/Fig1B-all_networks.png"), pB, width = 6, height = 4)
...
ggsave(here::here("plots/Fig1C-pairwise_competition.png"), pC, width = 3, height = 4)
...
ggsave(here::here("plots/Fig1.png"), p, width = 15, height = 6)
...
p_net_matrix <- net %>%
    ...
    draw_image(here::here("plots/cartoons/Fig1B_2.png"), x = 2.5, y = 4, vjust = -0.6, hjust = 0, clip = "on", scale = 1) +
    draw_image(here::here("plots/cartoons/Fig1B_3.png"), x = 1.5, y = 4, vjust = -0.6, hjust = 0, clip = "on", scale = 1) +
...
ggsave(here::here("plots/Fig2A-example_matrix.png"), pA, width = 4, height = 4)

# Figure 2B. all matrices
p_net_matrix_list <- lapply(net_list, function(x) plot_adjacent_matrix(x) + theme(plot.margin = grid::unit(c(5,0,3,0), "mm")))
## Subset the self-assembly networks
p_net_matrix_list <- p_net_matrix_list %>% `[`(communities %>% arrange(CommunitySize) %>% filter(str_detect(Community, "C")) %>% pull(Community))
p_net_list <- p_net_list %>% `[`(communities %>% arrange(CommunitySize) %>% filter(str_detect(Community, "C")) %>% pull(Community))
pB_title <- ggdraw() +
...
pB <- plot_grid(pB_title,
    ...
    paint_white_background()
ggsave(here::here("plots/Fig2B-matrices.png"), pB, width = 8, height = 4)
...
ggsave(here::here("plots/Fig2C-hierarchy.png"), pC, width = 3, height = 4)


# Figure 2D: motif of one network
## title
...
comm <- "C11R2"
motif_randomized_subset <- networks_motif_randomized %>% filter(Community == comm)
motif_community_subset <- networks_motif %>% filter(Community == comm)
plot_motif_count <- function(motif_randomized_subset, motif_subset = c(1,2,3)) {
...
ggsave(here::here("plots/Fig2E-diagonal_analysis.png"), pE, width = 8, height = 6)
...
#====================================================================================================
# Components in experimetns and simulations
# Components in experiment
...
# Observation
networks_component_summary <- networks_component %>%
    filter(str_detect(Community, "C\\d")) %>%
    group_by() %>%
    summarize(Component = sum(Component))
# Permutation
networks_component_randomized_summary <- networks_component_randomized %>%
    filter(str_detect(Community, "C\\d")) %>%
    group_by(Replicate) %>%
    summarize(Component = sum(Component))

p1 <- networks_component_randomized_summary %>%
    ggplot() +
    geom_histogram(aes(x = Component, color = "permutation"), binwidth = 1, color = 1, fill = NA) +
    geom_vline(data = networks_component_summary, aes(color = "observation", xintercept = Component)) +
    scale_color_manual(values = c("observation" = "red", "permutation" = "black")) +
    theme_classic() +
...
# Observation
df_component_summary <- df_component %>%
    group_by() %>%
    summarize(Component = sum(Component))
# Permutation
df_component_randomized_summary <- df_component_randomized %>%
    group_by(Replicate) %>%
    summarize(Component = sum(Component))

#
p2 <- df_component_randomized_summary %>%
    ggplot() +
    geom_histogram(aes(x = Component, color = "permutation"), binwidth = 1, color = 1, fill = NA) +
    geom_vline(data = df_component_summary, aes(color = "observation", xintercept = Component)) +
    scale_color_manual(values = c("observation" = "red", "permutation" = "black")) +
    theme_classic() +
    theme(panel.border = element_rect(color = 1, fill = NA),
          legend.title = element_blank(),
...
p <- plot_grid(p1, p2, nrow = 1, axis = "tblr", align = "h", scale = .8, rel_widths = c(1, 1.5), labels = c("Experiment", "Simulation")) + paint_white_background()
ggsave(here::here("plots/Fig1S17-components.png"), p, width = 8, height = 4)


#=================================================================================================================================================
# Network of random assembly
# Figure A. Matrices
p_net_matrix_list <- lapply(net_list, function(x) plot_adjacent_matrix(x) + theme(plot.margin = grid::unit(c(5,0,3,0), "mm")))
...
pB <- pairs %>%
    ...
    ggplot() +
    geom_col(aes(x = Assembly, y = Count, fill = InteractionType), color = 1, position = position_dodge(width = .8), width = .8) +
    #geom_text(aes(x = Assembly, label = paste0(round(Fraction, 2)* 100, "%")), y = Inf, size = 5, position = position_dodge(width = .8)) +
    #geom_text(aes(x = InteractionType, y = Count, label = paste0(round(Fraction, 3) * 100,"%")), nudge_y = 10, size = 5) +
    scale_fill_manual(values = assign_interaction_color(level = "simple")) +
...
# Figure 2D: motif of all randomassembled networks
## motif diagram
load(here::here("data/output/motif_list.Rdata"))
p_motif_list <- lapply(motif_list, function(x) plot_competitive_network(x, node_size = 2, edge_width = 1))
p1 <- plot_grid(plotlist = p_motif_list, nrow = 1, greedy = T)
...
motif_randomized_summary <- networks_motif_randomized %>%
    filter(str_detect(Community, "Ass")) %>%
...
p2 <- motif_randomized_subset %>%
    ...
    geom_hline(yintercept = 0, color = 1, linetype = 2) +
    geom_histogram(aes(y = Fraction, x = after_stat(count / max(count))), alpha = .3, color = 1) +
    geom_point(data = motif_community_subset, aes(x = 0, y = Fraction, color = "observed network"), pch = 1, size = 2, stroke = 2, inherit.aes = F) +
    scale_fill_manual(values = c("head" = "#FF0000A0", "body" = "#A0A0A0A0", "tail" = "#FF0000A0"),
                      labels = c("head" = "top 5%", "body" = "middle", "tail" = "bottom 5%")) +
    scale_color_manual(values = c("observed network" = "red")) +
    facet_grid(.~Motif) +
...
pD <- plot_grid(p1, p2, ncol = 1, rel_heights = c(1, 3), axis = "lr", align = "v") + paint_white_background()
...
#
p_top <- plot_grid(pA, pB, nrow = 1, rel_widths = c(2,1), scale = .9)
p_bottom <- plot_grid(pC, pD, pE, nrow = 1, labels = LETTERS[2:4], rel_widths = c(1,2,1.5), scale = c(.8, .9, .9), axis = "t", align = "h")
p <- plot_grid(p_top, p_bottom, nrow = 2, labels = c("A", ""), scale = c(1, 1),
               rel_heights = c(1,2)) + paint_white_background()
ggsave(here::here("plots/Fig2S0-random_networks.png"), p, width = 12, height = 6)
...
# Figure 1S4: cartoon of self-assembly experiment and isolate abundance
