...
ggsave(here::here("plots/FigS2-species_fitness_all_transfers_linear.png"), p, width = 12, height = 15)
...
p <- communities_abundance_fitness_all %>%
    ...
    # geom_smooth(data = filter(communities_abundance_fitness_all, CommunityESV %in% ESV_sig$CommunityESV),
    #             aes(x = Relative_Abundance, y = Fitness), method = "lm", formula = y~x, se = F) +
    geom_smooth(data = communities_abundance_fitness_all %>% filter(Community %in% communities$Community) %>% left_join(ESV_sig) %>% replace_na(list(Significance = "p>=0.05")),
                aes(x = Relative_Abundance, y = Fitness, color = Significance),
                method = stats::lm, formula = y ~ x, se = F) +
    geom_point(aes(x = Relative_Abundance, y = Fitness), shape = 21) +
    geom_hline(yintercept = 0, linetype = 2) +
...
ggsave(here::here("plots/FigS2-species_fitness_all_transfers_linear_comm.png"), p, width = 8, height = 6)
...
ggsave(here::here("plots/FigS2-species_fitness_all_transfers_linear_ephermeral.png"), p, width = 12, height = 9)
