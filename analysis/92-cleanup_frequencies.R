...
for (j in 1:length(batch_names)) {
    image_names <- list.files(paste0(folder_main, "images/", batch_names[j], "-07-feature/green")) %>%
        # Remove folders
        str_subset(".csv") %>%
        #str_subset("T8") %>%
        str_replace(".csv", "") %>%
    ...
    temp[[j]] <- tibble(image_name = image_names) %>%
        rowwise() %>%
        mutate(ColonyCount = paste0(folder_main, "images/", batch_names[j], "-07-feature/green/", image_name, ".csv") %>%
...
for (i in 1:nrow(pairs_freq_ID)) {
    ...
    temp[[i]] <- read_csv(paste0(folder_main, "images/", pairs_freq_ID$Batch[i],"-09-random_forest/", pairs_freq_ID$image_name_pair[i], ".csv"), show_col_types = F)
