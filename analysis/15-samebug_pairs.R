...
#' in commandline where 00-16s_mismatch.py is stored, execute `python 14-pairwise_16s_mismatch.py /Users/cychang/Dropbox/lab/emergent-coexistence/data/`
#' the output matrix is saved as ~/Dropbox/lab/emergent-coexistence/data/temp/14-mismatch_matrix_communities.csv
#' where the column and row names are the Sanger ID


# 3. Match the mismatch matrix to my internal ID for isoaltes
mismatch_matrix <- read_csv(paste0(folder_data, "temp/14-mismatch_matrix_communities.csv"), show_col_types = F) %>%
    pivot_longer(cols = -`...1`) %>%
...
write_csv(pairs_mismatch, paste0(folder_data, "temp/15-pairs_mismatch.csv"))
