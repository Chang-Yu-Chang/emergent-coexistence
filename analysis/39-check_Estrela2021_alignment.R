...
isolates_ID_RDP <- fread(paste0(folder_data, "raw/test/Estrela_2021_isolates_16S_taxa.csv"))
communities_abundance_syl <- fread(paste0(folder_data, "raw/test/Goldford_2018_16S_data_table_glucose_RelAbund_ALL.csv"))
as_tibble(isolates_ID_RDP) %>%
    dplyr::filter(Community %in% communities$Community) %>%
    arrange(Community) %>%
    group_by(Community) %>%
...
sequences_abundance <- sequences_alignment_syl %>%
