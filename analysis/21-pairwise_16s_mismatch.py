...
def calculate_mismatches(seq1,seq2,t=20):
    ...
    SeqIO.write(seq1, folder_data + 'temp/21-needle/alpha.faa','fasta')
    SeqIO.write(seq2, folder_data + 'temp/21-needle/beta.faa','fasta')
    needle_cline = NeedleCommandline(asequence = folder_data + 'temp/21-needle/alpha.faa', bsequence = folder_data + 'temp/21-needle/beta.faa', gapopen = 10, gapextend = 0.5, outfile = folder_data + 'temp/21-needle/needle.txt')
    stdout, stderr = needle_cline()
    align = AlignIO.read(folder_data + 'temp/21-needle/needle.txt', 'emboss')
...
for file in glob.glob("*F.ab1"):
    ...
    plt.savefig(folder_data + 'temp/21-needle/' + "%s%s" % (name,var3))
    plt.clf()
SeqIO.write(merged_sequence_list, folder_data + 'temp/21-needle/Merged_Sequences.fasta', 'fasta')
merged_sequence_list = list(SeqIO.parse(folder_data + 'temp/21-needle/Merged_Sequences.fasta', 'fasta'))
...
df.to_csv(folder_data + 'temp/21-mismatch_matrix_communities.csv')	
