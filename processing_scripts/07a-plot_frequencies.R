#' This script reads the aggregated csv from 06-bootstrapping and plot the results for each coculture
#'
#' B-09-bootstrap/
#' C-09-bootstrap/
#' C2-09-bootstrap/
...
for (i in 1:nrow(pairs_ID)) {
    ...
    p1 <- pair_boots_i %>%
        ...
        geom_line(aes(x = Time, y = Isolate1CFUFreq, color = FreqChangeSign, group = BootstrapID), linewidth = .1) +
