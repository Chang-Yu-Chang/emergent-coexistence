...
```{r}
plates_random <- read_csv(here::here("data/output/plates_random.csv"))
plates_random
```
...
```{r}
CASEU_RN1 <- read_csv(here::here("data/temp/CASEU_RN1.csv"))
```
...
```{r}
source("script/02B-random_network_CASEU-02-batch2.R")
```
...
```{r}
if (run_scripts) source("script/02B-random_network_CASEU-02-batch4.R")
```
...
## Random network, caseu batch 5

- Genewiz submission code: 
- Plate layout:
    - T0 BD P2
...
write_csv(genewiz_table, file = here::here("output/protocol/tab_fig/20211202_Sanger_seq_prep-genewiz_table_CYC.csv"))
...
Plus the across-community and random networks (20211108 in progress)
...
pairs_freq %>%
    ...
    mutate(Time = ordered(Time, c("T0", "T8")), Isolate1InitialODFreq = factor(Isolate1InitialODFreq)) %>%
