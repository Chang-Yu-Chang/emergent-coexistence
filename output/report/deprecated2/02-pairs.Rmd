...
```{r}
if (write_all_csv) {
  fwrite(pairs, root$find_file("data/output/pairs.csv"))
  fwrite(pairs_melted, root$find_file("data/output/pairs_melted.csv"))
}
```
...
# Combine plot
p_pairs_interaction_cb <- plot_grid(p_pairs_interaction, p_pairs_example_outcomes, 
  ncol = 1, align = "v", rel_heights = c(1, 0.5))
```

```{r fig.width = 4, fig.height = 3}
p_pairs_interaction_cb
pdf(root$find_file("output/figure/pairs_outcomes.pdf"), width = 8, height = 6); p_pairs_interaction_cb; invisible(dev.off())
```
...
p_pairs_interaction_finer <- plot_bar(pairs, bar_by = "InteractionTypeFiner", show_fill_legend = T) +
  geom_text(data = data.frame(InteractionTypeFiner = "neutrality", x = 1, y = 1, label = "n = 186 pairs"), mapping = aes(x = x, y = y, label = label)) +
  theme(legend.position = "top")

# Combine plot
theme_set(theme_cowplot(font_size=12))
p_pairs_interaction_finer_cb <- plot_grid(p_pairs_interaction_finer, p_pairs_example_outcomes_finer, ncol = 1, align = "v", rel_heights = c(1, 0.4))
```

```{r fig.width = 5, fig.height = 3}
p_pairs_interaction_finer_cb
pdf(root$find_file("output/figure/pairs_outcomes_finer.pdf"), width = 10, height = 6); p_pairs_interaction_finer_cb; invisible(dev.off())
```
...
pairs_interaction_fermenter
p_pairs_interaction_fermenter
pdf(root$find_file("output/report/figure/02-pairs-fermenter_interaction.pdf"), width = 5, height = 5); p_pairs_interaction_fermenter; invisible(dev.off())
...
```{r fig.width=3, fig.height=3}
pairs_interaction_family
p_pairs_interaction_family
pdf(root$find_file("output/report/figure/02-pairs-family_interaction.pdf"), width = 5, height = 5); p_pairs_interaction_family; invisible(dev.off())
```
...
## Pairwise distance difference of tips on a tree

Pairwise tree distance differnce
