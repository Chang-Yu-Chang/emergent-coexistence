...
There are four isolates that have either 0 CFU or negative OD value, so they don't have epsilon values.
...
Write isolate epsilons to `data/temp/isolates_epsilon_uncertainty.csv`

```{r}
if (write_all_csv) fwrite(isolates_epsilon_uncertainty, root$find_file("data/temp/isolates_epsilon_uncertainty.csv"))
```
...
```{r}
if (write_all_csv) fwrite(pairs_CFU_freq, file = root$find_file("data/temp/pairs_CFU_freq.csv"))
```
...
if (write_all_csv) fwrite(pairs_CFU_freq_uncertainty, file = root$find_file("data/temp/pairs_CFU_freq_uncertainty.csv"))
```
