...
library(data.table)
library(ggtree)
root <- rprojroot::is_r_package # Package root directory
source(root$find_file("misc/utils.R"))
write_all_csv <- TRUE # Whether to write csv file in this analysis Rmd
write_all_pdf <- TRUE # Whether to write pdf file in this analysis Rmd
...
```{r}
if (write_all_csv) {
  fwrite(isolates, root$find_file("data/output/isolates.csv"))
  fwrite(isolates_melted, root$find_file("data/output/isolates_melted.csv"))
}
```
