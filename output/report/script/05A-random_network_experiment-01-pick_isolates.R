...
head_isolates_RDP <- filter(isolates, grepl("JVN", ExpID))
jean_isolates_RDP <- fread(here::here("data/temp/jean_isolates_RDP.csv"))

# Read alignment data
# pairs_sanger_alignment <- fread(here::here("data/temp/pairs_sanger_alignment.csv"))
# jean_pairs_sanger_alignment <- fread(here::here("data/temp/jean_pairs_sanger_alignment.csv"))
