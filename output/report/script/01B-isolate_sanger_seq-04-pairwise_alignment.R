...
# Read data
isolates_RDP <- fread(root$find_file("data/temp/isolates_RDP.csv")) %>% as_tibble()
jean_isolates_RDP <- fread(root$find_file("data/temp/jean_isolates_RDP.csv")) %>% as_tibble()


# Align self-assembled communities isolates
## Call alignment function
source(root$find_file("misc/sequence_pairwise_alignment.R"))
