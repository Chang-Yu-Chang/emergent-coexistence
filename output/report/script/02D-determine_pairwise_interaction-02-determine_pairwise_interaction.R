...
pairs_interaction_fitness <- pairs_freq_spread %>%
  ...
  spread(key = Isolate1InitialODFreq, value = DifferenceT8T0)

pairs_interaction_fitness$FreqFunc <- paste(pairs_interaction_fitness$`5`, pairs_interaction_fitness$`50`, pairs_interaction_fitness$`95`, sep = "_")

pairs_interaction_fitness <- pairs_interaction_fitness %>%
  select(Community, Isolate1, Isolate2, FreqFunc) %>%
  mutate(FreqFunc = ordered(FreqFunc, levels = paste(rep(c(1, -1, 0), each = 9), rep(rep(c(1, -1, 0), each = 3), 3), rep(c(1, -1, 0), 9), sep = "_"))) %>%
  arrange(Community, Isolate1, Isolate2, FreqFunc)
...
pairs_interaction_T8_freq <-
  ...
  group_by(Community, Isolate1, Isolate2) %>%
  spread(key = Isolate1InitialODFreq, value = Isolate1MeasuredFreqT8) %>%
  as.data.table()

pairs_interaction_T8_freq$PairIsolate1MeasuredFreqT8 <-
  paste(round(pairs_interaction_T8_freq$`5`, 2),
    round(pairs_interaction_T8_freq$`50`, 2),
    round(pairs_interaction_T8_freq$`95`, 2), sep = "_")

### Update the column `Isolate1Win`, which indicates the competitive exclusion that cannot be specified by fitness functions
pairs_interaction_T8_freq[PairIsolate1MeasuredFreqT8 == "1_1_1","Isolate1Win"] <- TRUE
pairs_interaction_T8_freq[PairIsolate1MeasuredFreqT8 == "0_0_0","Isolate1Win"] <- FALSE
...
interaction_type$FreqFunc <- paste(interaction_type$FromRare, interaction_type$FromMedium, interaction_type$FromAbundant, sep = "_")
...
pairs_interaction_fitness[!(FreqFunc %in% c("1_1_1", "-1_-1_-1")), c("From", "To") := list(Isolate1, Isolate2)]
