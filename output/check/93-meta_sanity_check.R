...
p3 <- plot_grid(p3a, p3b, nrow = 1, axis = "tblr", align = "h", scale = .9) +
    theme(plot.background = element_rect(fill = "white", color = NA))
ggsave(paste0(folder_main, "meta/comparison-total_count.png"), p3, width = 8, height = 4)


# Frequency
## Overall
p4 <- pairs_freq_combined %>%
    ggplot() +
    geom_abline(slope = 1, intercept = 0, color = "red", linetype = 2) +
    geom_hline(yintercept = c(0,1), color = gray(.8), linetype = 2) +
    geom_vline(xintercept = c(0,1), color = gray(.8), linetype = 2) +
    geom_point(aes(x = Isolate1FinalFraction_human, y = Isolate1FinalFractionMean_machine),
    ...
    facet_grid(.~PairType) +
    theme_classic()
ggsave(paste0(folder_main, "meta/comparison-coculture_frequency.png"), p4, width = 10, height = 4)
