...
def calculate_mismatches(seq1,seq2,t=20):
    ...
    SeqIO.write(seq1,'../Temp/alpha.faa','fasta')
    SeqIO.write(seq2,'../Temp/beta.faa','fasta')
    needle_cline = NeedleCommandline(asequence="../Temp/alpha.faa", bsequence="../Temp/beta.faa", gapopen=10, gapextend=0.5, outfile="../Temp/needle.txt")
    stdout, stderr = needle_cline()
    align = AlignIO.read("../Temp/needle.txt", "emboss")
...
def duplicate_removal(mismatch_matrix,sequences,threshold=0):
    ### Remove duplicates based on mismatch. Duplicates are pairs of strains with <= threshold mismatches. Iterative pipeline which seeks to maximize the number of unique sequences
    while sum(np.sum(mismatch_matrix<=threshold)>1):
        # Find sequences with most duplicates
        tseq = mismatch_matrix.index[np.sum(mismatch_matrix <=threshold) == np.max(np.sum(mismatch_matrix <=threshold))] 
        # Find which is shortest
        sequence_length =  [[len(x) for x in sequences if x.id == str(y)][0] for y in tseq]
        rem = [tseq[i] for i in range (0,len(tseq)) if sequence_length[i] == np.min(sequence_length)][0]
        # Remvoe shortest duplicates
        mismatch_matrix =mismatch_matrix.drop(str(rem),axis=0).drop(str(rem),axis=1) 	
...
def main(dir):
    ...
    for file in glob.glob("*F.ab1"):
        ...
        plt.savefig('../Merged_Probabilities/' + "%s%s" % (name,var3))
        plt.clf()
    SeqIO.write(merged_sequence_list,'../../Merged_Sequences.fasta','fasta')
    ...
    df.to_csv('../mismatch_matrix_raw.csv')	
