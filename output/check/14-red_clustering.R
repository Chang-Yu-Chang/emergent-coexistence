...
list_image_mapping_folder <- list_image_mapping %>%
    left_join(select(list_images, image_name_pair = image_name, folder_feature_pair = folder_red_feature, folder_red_cluster), by = "image_name_pair") %>%
    left_join(select(list_images, image_name_isolate1 = image_name, folder_feature_isolate1 = folder_red_feature), by = "image_name_isolate1") %>%
    left_join(select(list_images, image_name_isolate2 = image_name, folder_feature_isolate2 = folder_red_feature), by = "image_name_isolate2")
...
for (i in 1:nrow(list_image_mapping_folder)) {
    ...
    folder_red_cluster <- list_image_mapping_folder$folder_red_cluster[i]
    ...
    ggsave(filename = paste0(folder_red_cluster, image_name, ".png"), plot = p, width = 10, height = 6)
