...
''' Python script for running merging paired sequences from Genewiz 
    ...
    direct to the directory of EMBOSS and run the following code in command line
    ` ./configure
    make
    make install
    `
...
if False:
    ...
    for file in glob.glob("*F.ab1"):
        ...
        plt.clf()
    SeqIO.write(merged_sequence_list,'../Temp/Merged_Sequences.fasta','fasta')
merged_sequence_list = list(SeqIO.parse("../Temp/Merged_Sequences.fasta", "fasta"))
...
df.to_csv('../Temp/mismatch_matrix_communities.csv')	
