...
for i in range(list_images.shape[0]):
    # File directory
    file_gray = list_images.iloc[i]['folder_blue'] + list_images.iloc[i]['image_name'] + '.tiff'
    file_rolled = list_images.iloc[i]['folder_blue_rolled'] + list_images.iloc[i]['image_name'] + '.tiff'
    
    # 
    image = io.imread(file_gray)
    image_rolled = rolling_ball_light(image)
    io.imsave(file_rolled, image_rolled)
    print("blue channel\t" + str(i) + "/" + str(list_images.shape[0]) + "\t" + list_images.iloc[i]['image_name'])
