...
for (i in 1:nrow(list_images)) {
    ...
    folder_blue_rolled <- list_images$folder_blue_rolled[i]
    folder_blue_watershed <- list_images$folder_blue_watershed[i]
    folder_blue_feature <- list_images$folder_blue_feature[i]

    #
    image_rolled <- readImage(paste0(folder_blue_rolled, image_name, ".tiff"))
    ...
    ## Second filter removing watershed objects that are too small to be colonies
    ...
    writeImage(colorLabels(image_watershed2), paste0(folder_blue_watershed, image_name, ".tiff"))
    cat("\twatershed")
    ...
    if (is_null(object_feature)) {
        #write_csv(object_feature, paste0(folder_blue_feature, image_name, ".csv"))
