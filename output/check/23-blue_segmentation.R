...
for (i in 1:nrow(list_images)) {
    ...
    image_rolled <- readImage(paste0(list_images$folder_blue_rolled[i], image_name, ".tiff"))
    ...
    save(image_watershed2, file = paste0(list_images$folder_blue_watershed_file[i], image_name, ".RData")) # save watersed image object
    writeImage(colorLabels(image_watershed2), paste0(list_images$folder_blue_watershed[i], image_name, ".tiff")) # save
    ...
    #     #write_csv(object_feature, paste0(list_images$folder_blue_feature[i], image_name, ".csv"))
