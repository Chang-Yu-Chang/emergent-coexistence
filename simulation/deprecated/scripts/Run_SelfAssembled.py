...
def run(input_row):
	...
	### Run coexisting pairs
	com_members = N_com.index[N_com.W0>1e-3].tolist()  
	pairs = list(combinations_with_replacement(com_members, 2))
	a['n_wells'] = len(pairs)
	#Set initial state
	N0,R0 = MakeInitialState(a)
	...
	Com = Plate.N[Plate.N>1e-3].dropna()
	N_Community =  pd.DataFrame({
		'Family' : [x[0] for x in Com.index], 
		'Species' : [x[1] for x in Com.index],
		'Abundance' : list(Com['W0']),
		'Supplied_Resource_Uptake' : [params['c'].loc[x][0] for x in Com.index],
		'By_Product_Uptake' : [np.sum(np.matmul(params['c'],params['D']).loc[x][0])  for x in Com.index]})

	N_Community.to_csv('../Data/Raw/Community_' + str(exp_id) + '.csv')
	N_Pairs.to_csv('../Data/Raw/Pairs_' + str(exp_id) + '.csv')
	return 0
