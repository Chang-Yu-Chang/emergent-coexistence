...
p1 <- cml %>%
    ...
    coord_cartesian(xlim = c(1, ma*fa), ylim = c(sa*fa, 1), clip = "off") +
    theme_classic() +
...
p2 <- Dml %>%
    ...
    guides(fill = guide_colorbar(title = expression(D[i][beta][alpha])),
           color = "none") +
    #labs(x = expression(R[alpha]), y = expression(R[beta])) +
    ggtitle("Metabolic matrix (D)")


ggsave(here::here("simulation/plots/21-matrix2_D.png"), p2, width = 6, height = 6)
